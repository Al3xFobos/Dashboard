...
export function Dashboard() {
  const { filters, setFilters, savedReports, saveCurrentReport, loadReport, deleteReport } =
    useDashboardStore();
    ...
    <div className="flex gap-6">
      <div className="flex-1 space-y-4">
        <header className="flex items-center justify-between">
          ...
          {isFetching && <Badge>Refreshing data…</Badge>}
        </header>
        ...
        <Card>
          <CardHeader className="flex items-center justify-between">
            ...
            <Badge className="text-xs">Brush to zoom · Hover for tooltips</Badge>
          </CardHeader>
...
<ResponsiveContainer width="100%" height="100%">
  <LineChart data={data?.timeSeries}>
    ...
    <Brush dataKey="date" height={20} stroke={chartLinePrimary} />
          ...
          <Card className="md:col-span-2">
            <CardHeader className="flex items-center justify-between">
              ...
              <span className="text-xs text-slate-500">
                Click row to drill down into the chart selection.
              </span>
            </CardHeader>
            <CardContent>
              {isLoading ? (
      ...
      <aside className="hidden w-72 flex-shrink-0 flex-col gap-3 md:flex">
        <Card>
          <CardHeader>
          ...
          <CardContent className="space-y-2">
            ...
            />
            <Button size="sm" className="w-full" onClick={handleSaveReport} disabled={!reportName}>
              Save report
            </Button>
          </CardContent>
        </Card>
        <Card className="flex-1">
          ...
          <CardContent className="space-y-2">
            {savedReports.length === 0 && (
              <p className="text-xs text-slate-500">
                Saved reports will appear here. Use them as quick presets for filters.
              </p>
            )}
      ...
      <CardContent className="flex flex-wrap items-center gap-3 py-3">
        <div className="space-y-1">
          ...
          <Select
            ...
            onChange={(e) => setFilters({ dateRange: e.target.value as DateRangePreset })}
          >
        ...
        <div className="space-y-1">
          ...
          <Select
            ...
            onChange={(e) => setFilters({ source: e.target.value as TrafficSource })}
          >
        ...
        <div className="space-y-1">
          ...
          <Select
            ...
            onChange={(e) => setFilters({ device: e.target.value as DeviceType })}
          >
...
function TopPagesTable({ data }: TopPagesTableProps) {
        ...
        <table className="w-full min-w-[300px] table-fixed border-collapse text-xs">
          <thead>
            <tr className="border-b border-border bg-muted/50 text-left">
              ...
              <th className="px-2 py-1 font-medium text-right">Conversions</th>
            ...
            {pages.map((p) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-2 py-1 text-right">{formatNumber(p.views)}</td>
                  <td className="px-2 py-1 text-right">{formatNumber(p.conversions)}</td>
                  <td className="px-2 py-1 text-right">{cr.toFixed(1)}</td>
                </tr>
              );
            })}
          </tbody>
  ...
  <YAxis tick={{ fontSize: 10, fill: chartAxisColor }} stroke={chartGridColor} />
  <Tooltip
    contentStyle={{
      background: chartTooltipBg,
      border: `1px solid ${chartTooltipBorder}`,
